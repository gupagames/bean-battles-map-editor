...
namespace GG.BeanBattles.MapEditor
{
    public static class EditorMapValidation
    {
        ...
        public static int MaxTriangles = 3000000;
        public static int MaxTextureDimension = 4096;
        public static int MaxRealtimeLights = 50;
        public static int MaxMeshCollidersVerts = 50000;
        ...
        public static bool ValidateMapSettings(EditorMapSettings settings)
        {
            try
            {
                ...
                for (int i = 0; i < settings.Stages.Length; i++)
                {
                    ...
                    // zone size is valid
                    if (stage.ZoneSize.x < MinZoneSize || stage.ZoneSize.y < MinZoneSize || stage.ZoneSize.z < MinZoneSize
                        || stage.ZoneSize.x > MaxZoneSize || stage.ZoneSize.y > MaxZoneSize || stage.ZoneSize.z > MaxZoneSize)
                    { Debug.LogError($"Failed to validate map settings, Stage[{i}] ZoneSize invalid: {stage.ZoneSize}, MAX {MaxZoneSize} MIN {MinZoneSize}"); return false; }
